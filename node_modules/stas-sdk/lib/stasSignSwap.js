...
async function unSigned(unSignedSwapHex, takerInputTxHex, ownerPublicKey, isZeroChange, ownerPrivateKey) {
    ...
    if (ownerIsStas) {
      for (let i = 0; i < offerTxObject.outputs.length; i++) {
        outputsUnlockingScriptPart += `${utility.convertToLESM(offerTxObject.outputs[i].satoshis)} ${offerTxObject.outputs[i].script.slice(6, 46)} `;
      ...
      if (!isZeroFee) {
        outputsUnlockingScriptPart += `${utility.convertToLESM(offerTxObject.inputs[2].outputIndex)} ${utility.convertToReverseEndian(offerTxObject.inputs[2].prevTxId)} `;
      }
      const makerPreimage = bsv.Transaction.Sighash.sighashPreimage(tx, utility.SIGHASH, 0, bsv.Script.fromHex(ownerScript), new BN(tx.outputs[1].satoshis), utility.FLAGS).toString('hex');
      outputsUnlockingScriptPart += `${utility.convertToLESM(takerVout)} ${takerInputTxHex} OP_1 ${makerPreimage} `;
