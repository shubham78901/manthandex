...
async function unSignedFunc(offerTxHex, takerPublicKey, makerInputTxHex, takerInputTxHex, takerVout, paymentPubKey, paymentUtxo, additionalOutputs, data, isZeroChange, ownerPrivateKey, paymentPrivateKey, txCost, feeEstimateCall = false) {
    ...
    for (let i = 0; i < outputData.length; i++) {
      outputsUnlockingScriptPart += `${utility.convertToLESM(tx.outputs[i].satoshis)} ${outputData[i].publicKeyHash} `;
    ...
    if (!isZeroFee) {
      outputsUnlockingScriptPart += `${utility.convertToLESM(paymentUtxo.vout)} ${utility.convertToReverseEndian(paymentUtxo.txid)} `;
    }
  
    const takerPreimage = bsv.Transaction.Sighash.sighashPreimage(tx, utility.SIGHASH, 1, bsv.Script.fromHex(takerInputUtxo.script), new BN(takerInputUtxo.satoshis)).toString('hex');
  
    if (makerIsStas) {
      const makerPreimage = bsv.Transaction.Sighash.sighashPreimage(tx, utility.SIGHASH, 0, bsv.Script.fromHex(makerInputUtxo.script), new BN(makerInputUtxo.satoshis), utility.FLAGS).toString('hex');
      const makerUnlockScript = `${outputsUnlockingScriptPart}${utility.convertToLESM(takerInputUtxo.vout)} ${takerInputTxHex} OP_1 ${makerPreimage}`;
    ...
    if (takerIsStas) {
      takerUnlockScript += `${outputsUnlockingScriptPart}${utility.convertToLESM(makerInputUtxo.vout)} ${makerInputTxHex} OP_1 ${takerPreimage} `;
    }
  
    if (ownerPrivateKey) {
      takerUnlockScript += `${bsv.Transaction.Sighash.sign(tx, ownerPrivateKey, utility.SIGHASH, 1, bsv.Script.fromHex(takerInputUtxo.script), new BN(takerInputUtxo.satoshis), utility.FLAGS).toTxFormat().toString('hex')} ${takerPublicKey}`;
      ...
      unsignedData.push({inputIndex: 1, satoshis: new BN(takerInputUtxo.satoshis), script: bsv.Script.fromHex(takerInputUtxo.script), sighash: utility.SIGHASH, publicKeyString: takerPublicKey, flags: utility.FLAGS, stas: true});
    }
  
    if (paymentPrivateKey) {
      tx.inputs[2].setScript(bsv.Script.fromASM(`${bsv.Transaction.sighash.sign(tx, paymentPrivateKey, utility.SIGHASH, 2, tx.inputs[2].output._script, new BN(paymentUtxo.satoshis), utility.FLAGS).toTxFormat().toString('hex')} ${paymentPubKey}`));
    } else {
